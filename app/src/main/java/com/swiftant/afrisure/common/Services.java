...
public class Services {
    ...
    public Services(Context context, RefreshListener refreshListener) {
        ...
        try {
            Cursor cursor = mydb.getTokenDetails();
            if(cursor != null && cursor.getCount() > 1) {
                token = cursor.getString(0);
                token = AESCrypt.decrypt(token);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
        ...
        try {
            activity.runOnUiThread(() -> {
                ...
                dialog.setPositiveButton("Ok", (dialog1, which) -> {
                    ...
                    Intent login = new Intent(activity, LoginPhoneNumber.class);
    ...
    public AlertDialog.Builder alertTheUser(String title, String message) {
